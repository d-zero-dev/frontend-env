...
describe('CLI', () => {
	test('dzbuild (no params)', async () => {
		...
		expect(
			...
			'htdocs/sub-folder.test/build.test/index.html {size} | gzip: {size}',
			'htdocs/sub-folder.test/css/style.css {size} | gzip: {size}',
			'htdocs/sub-folder.test/js/script.js {size} | gzip: {size}',
			'[11ty] Wrote 16 files in {time} ({time} each, {version})',
			...
			'sub-folder.test/build.test.pug -> sub-folder.test/build.test/index.html -> sub-folder.test/build.test.html',
		]);
	});
});
