...
- 「コミット」を求められた場合:
  ...
  - **重要: `main` にいる場合、コミットを進めず、ユーザーにトピックブランチ作成を提案する**（`dev` ブランチでの作業は許可されている）
  - **重要: 必ず `git status` で現在の状態を確認すること**
  - **重要: 以前の状態やメモリを信用しない — 必ずステージングエリアの現状を確認**
  1. ステージ済みファイルがある場合:
     - **重要: `git add` や `git restore` を絶対に使わずステージングエリアを一切変更しない**
     - `git diff --staged` でステージ済みファイルを確認し、ステージ済みファイル*のみ*を基にコミットメッセージを作成する
