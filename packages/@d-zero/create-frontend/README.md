...
| `--ignore-document-root` | -      | Document Root ファイルを `.gitignore` の追跡対象から外します。`--no-ignore-document-root` で追跡対象に含めます。 | `true`                    |
...
生成される `.gitignore` は `@d-zero/scaffold` のテンプレートをもとに、`--install` オプション有効時にセクションの取捨選択が行われます。デフォルトでは `# Document Root` セクションが保持され、Document Root 配下のファイルは追跡対象外になります。`--no-ignore-document-root` を指定するとこのセクションが除去され、Document Root ファイルが追跡対象になります。
